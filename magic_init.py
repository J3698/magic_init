...
def initializeWeight(D, type, N_OUT):
        # Here we first whiten the data (PCA or ZCA) and then
        # optionally run k-means on this whitened data.
        import numpy as np
        print(D.shape)
        if D.shape[0] < N_OUT:
        ...
        for i, (n, l) in enumerate(zip(net._layer_names, net.layers)):
                ...
                if len(l.blobs) > 0:
                        if np.sum(np.abs(l.blobs[0].data)) <= 1e-10:
                                ...
                                # Fill the parameters
                                inputs = {b: active_data[b] for b in net.bottom_names[n]}
                                initializeLayer(net, i, inputs, net.top_names[n][0], bias, type, max_data=max_data)
                        else:
